...
def generate_html(data: dict) -> str:
    """Generates the premium Slide-Deck HTML report for SEO (no Jinja2 — pure inline f-string)."""
    try:
        ...
        def status_meta(status):
            if status == "pass":
                return ("✅", "All Good", "#16a34a", "#f0fdf4", "#bbf7d0")
            if status == "warning":
                return ("⚠️", "Needs Fix", "#d97706", "#fffbeb", "#fde68a")
            if status == "fail":
        ...
        for idx, cat in enumerate(categories):
            ...
            for item in cat["items"]:
                emoji, lbl, clr, bg, border = status_meta(item["status"])
            ...
            <div class="check-card" style="background:{bg}; border-color:{border};">
              <div class="check-card-top">
                ...
                <span class="check-badge" style="background:{clr};">{lbl}</span>
              </div>
            </div>"""
...
<div class="deck" id="deck">
  ...
  <div class="slide cover active" data-slide="1">
    <div class="slide-inner">
      ...
      <div class="cover-meta">Report generated on {timestamp}</div>
      ...
      <div class="score-showcase">
        ...
        <div class="score-words">
          ...
          <div class="stat-row">
            <div class="stat-chip g">✓ {p} Passed</div>
            <div class="stat-chip a">⚠ {w} Warnings</div>
            <div class="stat-chip r">✗ {f} Failed</div>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="slide" data-slide="2">
    <div class="slide-inner">
      ...
      <div class="summary-sub">
        Out of <strong>{total} total checks</strong> we ran on your website, here's how things
        broke down. We'll walk you through each one in the next slides.
      </div>

      <div class="donut-row">
        <div class="donut-card g">
          <div class="donut-num">{p}</div>
          <div class="donut-lbl">✅ Things working well</div>
        </div>
        <div class="donut-card a">
          <div class="donut-num">{w}</div>
          <div class="donut-lbl">⚠️ Could be improved</div>
        </div>
        <div class="donut-card r">
          <div class="donut-num">{f}</div>
          <div class="donut-lbl">❌ Need fixing now</div>
        </div>
      </div>
      ...
      <div class="meaning-grid">
        <div class="meaning-card g">
          <div class="meaning-icon">✅</div>
          <div class="meaning-head">Working Well</div>
          <div class="meaning-body">These are set up correctly and actively helping your Google rankings.</div>
        </div>
        <div class="meaning-card a">
          <div class="meaning-icon">⚠️</div>
          <div class="meaning-head">Could Be Better</div>
          <div class="meaning-body">Not broken, but fixing these could noticeably improve your visibility.</div>
        </div>
        <div class="meaning-card r">
          <div class="meaning-icon">❌</div>
          <div class="meaning-head">Needs Fixing</div>
          <div class="meaning-body">These issues are actively hurting your chances of ranking on Google.</div>
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="slide end-slide" data-slide="{total_slides}">
    <div class="slide-inner">
      <div class="cover-eyebrow"
           style="background:rgba(255,255,255,0.08);border-color:rgba(255,255,255,0.15);color:rgba(255,255,255,0.65);">
        <span>🎯</span> REPORT COMPLETE
      </div>
      <div class="end-title">That's your full<br>SEO health check.</div>
      <div class="end-sub">
        You've reviewed all {total} checks across {num_cats} categories.
        Start with the ❌ red items first — they have the biggest impact on your Google rankings.
        Then work through the ⚠️ warnings.
      </div>
      <div class="end-score-badge">
        <div class="end-score-num">{score}/100</div>
        <div>
      ...
      <div class="end-footer">Generated using RankMath SEO Analyzer · {timestamp}</div>
    </div>
  </div>
