...
from app.models import User, Audit, BulkJob
from app.schemas import AnalysisRequest, AnalysisResponse, ReportType
from app.auth import get_password_hash, verify_password, create_access_token, get_current_user, get_super_admin, get_optional_user
from app.services.seo_service import scrape_rankmath_async, generate_html as gen_seo_html
from app.services.speed_service import fetch_pagespeed_data_sync, parse_pagespeed_data, generate_html as gen_speed_html
...
def admin_create_user(email: str, password: str, db: Session = Depends(get_db), admin_username: str = Depends(get_super_admin)):
...
async def bulk_analyze(
    ...
    background_tasks.add_task(process_bulk_audit, db, job.id, file_path, current_user.id)
    
    return {"job_id": job.id, "status": "queued"}

@app.get("/bulk/status/{job_id}")
def get_bulk_status(job_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def analyze_website(
    ...
    if report_type in [ReportType.SEO, ReportType.BOTH]:
        ...
        if gen_seo_html:
            ...
            with open(os.path.join(REPORTS_DIR, f"{report_id}_seo.html"), "w", encoding="utf-8") as f:
                f.write(html)
    ...
    if report_type in [ReportType.SPEED, ReportType.BOTH]:
        ...
        if raw_speed:
            ...
            if gen_speed_html:
                ...
                with open(os.path.join(REPORTS_DIR, f"{report_id}_speed.html"), "w", encoding="utf-8") as f:
                    f.write(html)
                await generate_pdf(html, os.path.join(REPORTS_DIR, f"{report_id}_speed.pdf"))
