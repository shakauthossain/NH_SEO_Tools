...
export default function App() {
  ...
  const [savedAdminCreds, setSavedAdminCreds] = useState<{user: string; pass: string} | null>(null);
  ...
  const handleAdminCreate = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(
        ...
        {},
        { auth: { username: savedAdminCreds.user, password: savedAdminCreds.pass } },
      );
  ...
  const handleAdminAuthSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Verify creds by fetching stats
      await api.get("/admin/stats", { auth: { username: adminAuthUser, password: adminAuthPass } });
      setSavedAdminCreds({ user: adminAuthUser, pass: adminAuthPass });
      setAdminLoggedIn(true);
      setIsAdminAuthOpen(false);
      fetchAdminData(adminAuthUser, adminAuthPass);
      // If there was a pending create action
        ...
        {isAdminAuthOpen && (
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-text-muted mb-8 text-sm">
                Please enter your Super Admin credentials to authorize this action.
              </p>
              ...
              <form onSubmit={handleAdminAuthSubmit} className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {adminAuthLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Authorize"}
                  </button>
                </div>
              </form>
            </motion.div>
                      ...
                      <div className="max-w-md mx-auto">
                        ...
                        <div className="bg-bg-card p-6 border border-border-subtle rounded-xl">
                          ...
                          <form onSubmit={handleAdminAuthSubmit} className="space-y-4">
                            ...
                            >
                              {adminAuthLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : "Sign In as Admin"}
                            </button>
                            {adminStatus && (
                        ...
                        {adminStats && (
                          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div className="bg-bg-card border border-border-subtle rounded-xl p-6">
                              ...
                              <span className="text-3xl font-bold text-text-main">{adminStats.total_users}</span>
                            </div>
                            <div className="bg-bg-card border border-border-subtle rounded-xl p-6">
                              <div className="flex items-center gap-3 mb-3">
                                <BarChart3 className="w-5 h-5 text-pass" />
                                <span className="text-xs font-bold text-text-muted uppercase tracking-wider">Total Audits</span>
                              </div>
                              <span className="text-3xl font-bold text-text-main">{adminStats.total_audits}</span>
                            </div>
                            <div className="bg-bg-card border border-border-subtle rounded-xl p-6">
                              <div className="flex items-center gap-3 mb-3">
                                <FileUp className="w-5 h-5 text-warn" />
                                <span className="text-xs font-bold text-text-muted uppercase tracking-wider">Bulk Jobs</span>
                              </div>
                              <span className="text-3xl font-bold text-text-main">{adminStats.total_bulk_jobs}</span>
                            </div>
                          </div>
                        )}
                            ...
                            <table className="w-full">
                              <thead>
                                <tr className="border-b border-border-subtle text-left">
                                  <th className="px-6 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">ID</th>
                                  <th className="px-6 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">Email</th>
                                  <th className="px-6 py-3 text-xs font-bold text-text-muted uppercase tracking-wider">Audits</th>
                                  <th className="px-6 py-3 text-xs font-bold text-text-muted uppercase tracking-wider text-right">Actions</th>
                                </tr>
                              </thead>
                              <tbody>
                                {adminUsers.map((u: any) => (
                                  <tr key={u.id} className="border-b border-border-subtle last:border-0 hover:bg-bg-hover transition-colors">
                                    <td className="px-6 py-4 text-sm text-text-muted">#{u.id}</td>
